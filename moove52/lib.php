 ...
 * @return string
 */
function theme_moove52_get_main_scss_content($theme) {
    global $CFG;
    ...
    // Moove scss.
    $moovevariables = file_get_contents($CFG->dirroot . '/theme/moove52/scss/moove/_variables.scss');
    $moove = file_get_contents($CFG->dirroot . '/theme/moove52/scss/default.scss');
    $security = file_get_contents($CFG->dirroot . '/theme/moove52/scss/moove/_security.scss');

    $lastpreset = '';
    if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_moove52', 'preset', 0, '/', $filename))) {
 ...
 * @return string
 */
function theme_moove52_get_extra_scss($theme) {
    // die('Extra SCSS Callback Working');
    $content = '';
 ...
 * @return string
 */
function theme_moove52_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 */
function theme_moove52_get_precompiled_css() {
    global $CFG;

    return file_get_contents($CFG->dirroot . '/theme/moove52/style/moodle.css');
}
 ...
 * @return mixed
 */
function theme_moove52_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    $theme = theme_config::load('moove52');
    ...
    if ($filearea === 'hvp') {
        return theme_moove52_serve_hvp_css($args[1], $theme);
    }
 ...
 */
function theme_moove52_serve_hvp_css($filename, $theme) {
    global $CFG, $PAGE;
    ...
    $settings = new \theme_moove52\util\settings();
    $content = $settings->hvpcss;

    $md5content = md5($content);
    $md5stored = get_config('theme_moove52', 'hvpccssmd5');
